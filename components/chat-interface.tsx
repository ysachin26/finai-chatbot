...
export default function ChatInterface({
  ...
  const processTransactionCommand = (message: string) => {
    ...
    const sendMatch = message.match(
      /send\s+(\d+(?:\.\d+)?)\s+XLM\s+to\s+([A-Z0-9]+)/i
    );
    const balanceMatch = message.match(
      /check\s+(?:my\s+)?(?:wallet\s+)?balance/i
    );
    const transactionHistoryMatch = message.match(
      /(?:show|view|get)\s+(?:my\s+)?(?:transaction|payment)\s+history/i
    );
    ...
    if (
      ...
      lastTransactions.forEach((tx: any, index: number) => {
        ...
        if (tx.type === "sent") {
          historyText += `${index + 1}. Sent ${tx.amount} XLM to ${
            tx.to
          } on ${date}\n`;
        } else {
          historyText += `${index + 1}. Received ${tx.amount} XLM from ${
            tx.from
          } on ${date}\n`;
        }
      });
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (!isOnline) {
      ...
      const offlineResponse = {
        id: `offline-${Date.now()}`,
        content:
          "Your message has been saved and will be processed when you're back online.",
        role: "assistant",
        createdAt: new Date(),
        offline: true,
      };
  ...
  return (
    <div className="flex flex-col h-full">
      {!isOnline && (
        <Alert
          variant="default"
          className="mb-4 bg-amber-50 border-amber-200 text-amber-800"
        >
          <WifiOff className="h-4 w-4" />
          <AlertTitle>Offline Mode</AlertTitle>
          <AlertDescription>
            You're currently offline. Messages will be queued and sent when you
            reconnect.
            {offlineMessages.length > 0 &&
              ` (${offlineMessages.length} message${
                offlineMessages.length > 1 ? "s" : ""
              } queued)`}
          </AlertDescription>
        </Alert>
      )}
            ...
            <div className="p-4 space-y-4">
              {messages.map((message) => (
                ...
                >
                  <div
                    className={cn(
                      "flex max-w-[80%] gap-3 rounded-lg px-4 py-2",
                      message.role === "user"
          ...
          <div className="border-t p-4 bg-white dark:bg-gray-950">
            <form onSubmit={handleFormSubmit} className="flex gap-2">
              ...
              <Button
                ...
                disabled={isLoading || !isOnline || micPermissionDenied}
                className={cn(
                  "h-10 w-10",
                  isVoiceInput && "text-red-500",
                  micPermissionDenied && "text-gray-400"
                )}
              >
